...
class Editable_kafeel extends CI_Controller {
        ...
        if (filter_input_array(INPUT_POST)) {
            ...
            if($input_name == 'phone_no'){
                ...
                if (!$this->form_validation->run($rules) == FALSE) {
                    ...
                    $success = $this->kafeel->updateRecord('uid', ['cell_number' => $value], $pk);
                    if($success){
                ...
                if (!$this->form_validation->run($rules) == FALSE) {
                    $success = $this->kafeel->updateRecord('uid', ['office_number' => $value], $pk);
                    if($success){
                        header("HTTP/1.1 200 OK");
                    }else{
                        header('HTTP/1.0 400 Bad Request', true, 400);
                ...
                if (!$this->form_validation->run($rules) == FALSE) {
                    $success = $this->kafeel->updateRecord('uid', ['address' => $value], $pk);
                    if($success){
                        header("HTTP/1.1 200 OK");
                    }else{
                        header('HTTP/1.0 400 Bad Request', true, 400);
